...
if (length(args) < 4) {
  cat("Usage: Rscript run_decontx.R <raw_tar_file_dir> <filtered_tar_file_dir> <sequencing_technology> <decontx_out_prefix> [--dont_prepend_sample_to_barcodes]\n")
  quit(status = 1)
}
...
dont_prepend <- ifelse("--dont_prepend_sample_to_barcodes" %in% args, TRUE, FALSE)
